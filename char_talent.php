...
function char_talent(&$sqlr, &$sqlc)
{
    ...
    $result = $sqlc->query('SELECT account, name, race, class, level, gender, (SELECT count(spell) FROM character_talent WHERE guid = '.$id.' AND talentGroup = (SELECT activeTalentGroup FROM characters WHERE guid = '.$id.')) AS talent_points
